...
class CFM(nn.Module):
    ...
    def sample_reverse(
        ...
        *,
        ...
        # 4. 构建反向的 Cond Padding (把 Ref Mel 放到最后)
        # 原始逻辑是 F.pad(..., (0, gen_len))，这里我们改成 (gen_len, 0)
        
        # 计算需要生成的长度 (Padding 长度)
        # 注意：这里假设 batch 中每个样本生成的长度可能不同，需要精细处理
        # 简单起见，这里演示 batch=1 或 等长 padding 的逻辑。
        # 如果是 variable length batch，建议在这里做更复杂的 mask 构建。
        
        # 先初始化一个全 0 的大底板 (Batch, Total_Len, Dim)
