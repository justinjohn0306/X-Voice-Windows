...
def process_batch(batch_data, lang_code, tokenizer_str, sft_gen_dir):
    ...
    for (audio_path, _, duration), ipa_text in zip(batch_data, ipa_texts):
        ...
        if not (os.path.exists(pt_path) and os.path.exists(json_path)):
            local_fail += 1
            print(f"[warn]: {rel_path_no_suffix} not exists!")
            continue
...
def read_all_metadata(input_dir):
    ...
    all_files = list(input_path.glob("metadata_th*_top_1000.0h_old.csv"))
    csv_files =[]
    for f in all_files:
        csv_files.append(f)
...
def prepare_all(inp_dir, out_dir_root, tokenizer, dataset_name, sft_gen_dir, num_workers=16, duration_map=None):
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        for lang_code, csv_path in tasks:
            ...
            # if True:
            #     print("正在使用单线程DEBUG模式运行，否则请注释掉")
                
            #     for batch in batches:
            #         batch_results = process_batch(batch, lang_code, tokenizer, sft_gen_dir)      
            #         for res in batch_results:
            #             print(res) 
            #         break
            
            # 将 sft_gen_dir 传给 process_batch
    ...
    with open(stats_path, "w", encoding="utf-8") as f:
        ...
        for token, stats in sorted_stats:
            lang_str = ",".join(sorted(list(stats['langs']))) 
