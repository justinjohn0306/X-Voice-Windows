# 修改这里的配置
# bash src/f5_tts/eval/eval_multilingual_seedtts.sh
asr_gpu=8
task=zero_shot
dataset=seedtts_testset # lemas_eval
ckpt=50000
...
if [ "$layered" = "True" ]; then
    ...
    accelerate launch --main_process_port 29508 src/f5_tts/eval/eval_infer_batch.py \
        ...
        --drop_text -ns "SpeedPredict_Multilingual" -cs 28000  #--reverse 
else
...
for lang in ${test_set}; do
    python src/f5_tts/eval/eval_seedtts_testset.py -e wer -l ${lang} --gen_wav_dir ${decode_dir} --gpu_nums ${asr_gpu} --local
    python src/f5_tts/eval/eval_seedtts_testset.py -e sim -l ${lang} --gen_wav_dir ${decode_dir} --gpu_nums ${asr_gpu} --local
    python src/f5_tts/eval/eval_utmos.py --audio_dir ${decode_dir}/${lang}
done
