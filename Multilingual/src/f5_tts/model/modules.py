...
class AdaLayerNorm(nn.Module):
    def __init__(self, dim, use_rmsnorm):
        super().__init__()
...
class SwiGLUFFNFused(SwiGLUFFN):
    def __init__(
        ...
        hidden_features = (int(dim* mult * 2 / 3) + 7) // 8 * 8
        super().__init__(
            in_features=dim,
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        context_mask: bool["b nt"] | None = None,
        rope=None,  # rotary position embedding for x
        c_rope=None,  # rotary position embedding for c
    ) -> torch.Tensor:
        if isinstance(self.processor,JointAttnProcessor):
            if c is not None:
                return self.processor(self, x, c=c, mask=mask, context_mask=context_mask, rope=rope, c_rope=c_rope)
            else:
                return self.processor(self, x, mask=mask, context_mask=context_mask, rope=rope)
        else:
            if c is not None:
...
class JointAttnProcessor:
    ...
    def __call__(
        ...
        if context_mask is not None and not attn.context_pre_only:
             context_mask_expanded = context_mask.unsqueeze(-1) # [B, Nt, 1]
             c = c.masked_fill(~context_mask_expanded, 0.0)
...
class DiTBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.attn_norm = AdaLayerNorm(dim, use_rmsnorm)
        if not use_rmsnorm:
        ...
        if use_swiglu:
            self.ff = SwiGLUFFNFused(dim=dim, mult=ff_mult, drop_out=dropout)
        else:
...
class TimestepEmbedding(nn.Module):
    ...
    def forward(self, timestep: float["b"]):
        ...
        return time
####################################################################################
class AdaLayerNorm_Frame(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.silu = nn.SiLU()
...
class M3DiTBlock(nn.Module):
    def __init__(
        ...
        self.attn_norm = AdaLayerNorm_Frame(dim)
    ...
    def forward(self, x, t, mask=None, rope=None):  # x: noised input, t: time embedding
        ...
        return x
class M3MMDiTBlock(nn.Module):
    r"""
    modified from MMDiTBlock
    """
    ...
    def __init__(
        ...
        super().__init__()
        if context_dim is None:# 文本流的维度
            context_dim = dim # 音频流的维度
        self.context_pre_only = context_pre_only

        self.attn_norm_c = AdaLayerNorm_Final(context_dim) if context_pre_only else AdaLayerNorm(context_dim)
        self.attn_norm_x = AdaLayerNorm_Frame(dim)
        self.attn = Attention(
            processor=JointAttnProcessor(),
            dim=dim,
            heads=heads,
    ...
    def forward(self, x, c, t_c, t_x=None, mask=None, context_mask=None, rope=None, c_rope=None):  
        # x: noised input[b,n,d], c: context[b,nt,d], 
        # t_c是全局条件 [b,d]
        # t_x是帧级条件 [b,n,d]
        if t_x is None:
            t_x = t_c.unsqueeze(1).expand(-1, x.shape[1], -1)
        # pre-norm & modulation for attention input
        if self.context_pre_only:
            norm_c = self.attn_norm_c(c, t_c)
        else:
            norm_c, c_gate_msa, c_shift_mlp, c_scale_mlp, c_gate_mlp = self.attn_norm_c(c, emb=t_c)
        # 都是[b,n,d]
        norm_x, x_gate_msa, x_shift_mlp, x_scale_mlp, x_gate_mlp = self.attn_norm_x(x, emb=t_x)

        # attention
        x_attn_output, c_attn_output = self.attn(x=norm_x, c=norm_c, mask=mask,context_mask=context_mask, rope=rope, c_rope=c_rope)
        ...
        # process attention output for input x
        # 不用再unsqueeze
        x = x + x_gate_msa * x_attn_output
        norm_x = self.ff_norm_x(x) * (1 + x_scale_mlp) + x_shift_mlp
        x_ff_output = self.ff_x(norm_x)
        x = x + x_gate_mlp * x_ff_output

        return c, x

######################################################################################
# TODO. 下面M3没有加入lighting dit的改动
...
class M3DiTBlock_new(nn.Module):
    def __init__(
        self,
        dim,
        heads,
        ...
        qk_norm=None,
        pe_attn_head=None,
        attn_backend="torch",  #TODO. 加入flash attn "torch" or "flash_attn"
        attn_mask_enabled=True,
        kernel_size=3, # meanaudio卷积核，设为1则等价于线性层
    ):
        super().__init__()
        ...
        if kernel_size == 1:
            self.ff = MLP(dim, int(dim * ff_mult))
        else:
            self.ff = ConvMLP(dim, int(dim * ff_mult), kernel_size=kernel_size, padding=padding)
        
        if kernel_size == 1:
            self.conv_after_attn = nn.Linear(dim, dim)
        else:
            self.conv_after_attn = ChannelLastConv1d(dim, dim, kernel_size=kernel_size, padding=padding)
    ...
    def pre_attention(self, x ,t, rope=None):
        if t.ndim == 2:
            t = t.unsqueeze(1)
        mod_params = self.adaLN_modulation(t)
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = mod_params.chunk(6, dim=-1)
        norm_x = modulate(self.attn_norm(x), shift_msa, scale_msa)
        
        query = self.attn.to_q(norm_x)
        key = self.attn.to_k(norm_x)
        value = self.attn.to_v(norm_x)
        
        # Reshape for multi-head attention
        query = rearrange(query, 'b n (h d) -> b h n d', h=self.attn.heads)
        key = rearrange(key, 'b n (h d) -> b h n d', h=self.attn.heads)
        value = rearrange(value, 'b n (h d) -> b h n d', h=self.attn.heads)
    ...
    # mean audio
    def post_attention(self, x, attn_output, mod_params, mask=None):
        gate_msa, shift_mlp, scale_mlp, gate_mlp = mod_params
        x = x + self.conv_after_attn(attn_output) * gate_msa

        norm_ff = modulate(self.ff_norm(x), shift_mlp, scale_mlp)
        ff_output = self.ff(norm_ff)
        
        x = x + ff_output * gate_mlp
        if mask is not None:
            x = x.masked_fill(~mask.unsqueeze(-1), 0.0)
        return x
    
    def forward(self, x, t, mask=None, rope=None):
        (q, k, v), mod_params = self.pre_attention(x, t, rope)
        if self.attn_mask_enabled and mask is not None:
            attn_mask = mask
            attn_mask = attn_mask.unsqueeze(1).unsqueeze(1)  # 'b n -> b 1 1 n'
        else:
        ...
        x = self.post_attention(x, attn_output, mod_params, mask)
        return x
        
class M3MMDiTBlock_new(nn.Module):
    ...
    def __init__(
        ...
        self.context_pre_only = context_pre_only
        # mean audio
        self.speech_block = M3DiTBlock_new(dim, heads, dim_head, ff_mult, dropout, kernel_size=3)
        self.text_block = M3DiTBlock_new(dim, heads, dim_head, ff_mult, dropout, kernel_size=1)
    ...
    def forward(self, x, c, t_c, t_x=None, mask=None, context_mask=None, rope=None, c_rope=None):  
        ...
        if t_x is None:
            t_x = t_c.unsqueeze(1).expand(-1, x.shape[1], -1)
        (x_q, x_k, x_v), x_mod_params = self.speech_block.pre_attention(x, t_x, rope)
        (c_q, c_k, c_v), c_mod_params = self.text_block.pre_attention(c, t_c, c_rope)
        # 拼接 QKV
        joint_q = torch.cat([x_q, c_q], dim=2)
        joint_k = torch.cat([x_k, c_k], dim=2)
        joint_v = torch.cat([x_v, c_v], dim=2)
        # 拼接 Mask
        if mask is not None or context_mask is not None:
            if mask is None: mask = torch.ones(x.shape[:2], device=x.device, dtype=torch.bool)
            if context_mask is None: context_mask = torch.ones(c.shape[:2], device=c.device, dtype=torch.bool)
            joint_mask = torch.cat([mask, context_mask], dim=1)
            # 扩展为 Attention Mask 格式 [B, 1, N_q, N_k]
            attn_mask = joint_mask.unsqueeze(1).unsqueeze(2)
        else:
            attn_mask = None
        attn_out = F.scaled_dot_product_attention(joint_q, joint_k, joint_v, attn_mask=attn_mask)
        attn_out = rearrange(attn_out, 'b h n d -> b n (h d)')

        # 拆分 Attention 输出
        x_attn_out = attn_out[:, :x.shape[1]] 
        c_attn_out = attn_out[:, x.shape[1]:]
        # Post-Attention: 分别处理
        x_out = self.speech_block.post_attention(x, x_attn_out, x_mod_params, mask=mask)
        c_out = None
        if not self.context_pre_only:
            c_out = self.text_block.post_attention(c, c_attn_out, c_mod_params, mask=context_mask)
        return c_out, x_out
