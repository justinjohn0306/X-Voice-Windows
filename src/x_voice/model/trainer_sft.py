...
from x_voice.model import CFM_SFT
from x_voice.model.dataset import DynamicBatchSampler, collate_fn_sft
from x_voice.model.utils import default, exists
...
class Trainer_SFT:
    ...
    def _expand_prompt_token_embedding_if_needed(self, state_dict: dict, target_state_dict: dict):
        target_key = "transformer.text_embed.text_embed.weight"
        if target_key not in target_state_dict:
            return

        target_weight = target_state_dict[target_key].detach().cpu()
        target_rows, target_dim = target_weight.shape

        for key, value in list(state_dict.items()):
            if not key.endswith(target_key):
                continue
            if value.ndim != 2 or value.shape[1] != target_dim:
                continue
            if value.shape[0] != target_rows - 1:
                continue

            new_weight = target_weight.clone()
            new_weight[: value.shape[0]] = value
            state_dict[key] = new_weight

            if self.is_main:
                print(
                    f"Expanded text embedding for key '{key}' from {tuple(value.shape)} to {tuple(new_weight.shape)}."
                )
    ...
    def load_checkpoint(self):
        ...
        current_model_state = self.accelerator.unwrap_model(self.model).state_dict()
        self._expand_prompt_token_embedding_if_needed(checkpoint["ema_model_state_dict"], current_model_state)
        ...
        if "update" in checkpoint or "step" in checkpoint:
            ...
            self._expand_prompt_token_embedding_if_needed(checkpoint["model_state_dict"], current_model_state)
            ...
            for batch in current_dataloader:
                ...
                with self.accelerator.accumulate(self.model):
                    ...
                    loss, cond, pred = self.model(
                        mel_spec, text=text_inputs, lens=mel_lengths, prompt_lens=prompt_mel_lengths, noise_scheduler=self.noise_scheduler,language_ids=language_ids,
                    )
                    del cond, pred
                ...
                if global_update % self.save_per_updates == 0 and self.accelerator.sync_gradients:
                    ...
                    if self.log_samples and self.accelerator.is_local_main_process:
                        ...
                        if self.use_total_text:
                            infer_text = [
                                text_inputs[0] + (["_ _"] if isinstance(text_inputs[0], list) else "_ _") + batch["text"][0]
                            ]
                        else:
                            infer_text = [text_inputs[0]] # [batch["text"][0]]
                        infer_ids=[current_ids[0]]
                        with torch.inference_mode():
                            generated, _ = self.accelerator.unwrap_model(self.model).sample(
                                cond=mel_spec[0][:ref_audio_len].unsqueeze(0),
                                text=infer_text,
                                duration=(ref_audio_len * 2 - prompt_mel_lengths[0]).unsqueeze(0),
                                steps=nfe_step,
