...
from x_voice.model import CFM_SFT, Trainer_SFT
...
def main(model_cfg):
    model_cls = hydra.utils.get_class(f"x_voice.model.{model_cfg.model.backbone}")
    ...
    model = CFM_SFT(
        transformer=model_cls(
            **model_arc, 
            sft=sft,
            text_num_embeds=vocab_size + (1 if sft else 0),
    ...
    trainer = Trainer_SFT(
        ...
        checkpoint_path=str(files("x_voice").joinpath(f"../../{model_cfg.ckpts.save_dir}")),
