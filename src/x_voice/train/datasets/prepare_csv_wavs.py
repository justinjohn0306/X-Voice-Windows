...
from x_voice.model.utils import convert_char_to_pinyin


PRETRAINED_VOCAB_PATH = files("x_voice").joinpath("../../data/Emilia_ZH_EN_pinyin/vocab.txt")
language = "french"


def is_csv_wavs_format(input_dataset_dir):
    fpath = Path(input_dataset_dir)
    metadata = fpath / f"metadata_{language}.csv"
    wavs = fpath / "wavs"
    ...
    with open(csv_file_path, mode="r", newline="", encoding="utf-8-sig") as csvfile:
        ...
        for row in reader:
            if len(row) == 2:
                ...
                audio_text_pairs.append((audio_file_path.as_posix(), text, None))
            elif len(row) == 3:
                audio_file = row[0].strip()  # First column: audio file path
                duration = row[1].strip() # Second column: duration
                text = row[2].strip()  
                audio_file_path = audio_file
                audio_text_pairs.append((audio_file_path.as_posix(), text, duration))
...
def prepare_csv_wavs_dir(input_dir, num_workers=None):
    ...
    metadata_path = input_dir / f"metadata_{language}.csv"
    ...
    processed = audio_path_text_pairs
    if not processed:
